...
//				addProximityAlert();
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		DISTANCE_INTERVAL = b.getInt("DISTANCE_INTERVAL");
		TIME_INTERVAL = b.getInt("TIME_INTERVAL");
//		RADIUS_INTERVAL = b.getInt("RADIUS_INTERVAL");
...
//		addProximityAlert();
	...
	public void onDestroy() {
		...
		this.unregisterReceiver(this.broadcastReceiver);//Se requiere quitar del registro al servicio si no el proceso falla, aunque se detiene el servicio 
	}
	...
	public void onLocationChanged(Location location) {
		Log.i(TAG, "Guardando punto en onLocationChanged " + location.toString());
		addLocationToDB(location);
	}
...
//		int sampleDistance = DISTANCE_INTERVAL;
//		int sampleInterval = TIME_INTERVAL * 1000 * 60; //10 segundos
		int sampleDistance = 1;
		int sampleInterval = 3000; //3 segundos
		Toast.makeText(getApplicationContext(), "Actualizaciones a una distancia de " + sampleDistance 
				       + " m e intervalos de " + sampleInterval + " ms.", Toast.LENGTH_LONG).show();
...
//	private void addProximityAlert(Location l) {
//		Log.i(TAG, "addProximityAlert()");
//		int vicinityRadius = RADIUS_INTERVAL;
